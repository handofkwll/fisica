...
def calculate_visibility(smec_opd_to_mpd, sky_cube, wn_axis,
  ...
  parallel):
    ...
    # multiply the sky cube by the collectors psfs
    psf1 = amplitude_beam_1
    psf2 = amplitude_beam_2
    f1 = sky_cube * numpy.conj(psf1) * psf2
    ...
    # calculate the u/v axis (u==v in this case)
    u = numpy.fft.fftfreq(nx, spatial_axis[1] - spatial_axis[0])

    for it,t in enumerate(obs_timeline.keys()):
...
#            print '..pointing errors are enabled, calculating the interferograms will take a long time'
        ...
        if not parallel:
            ...
            # now tackle the timeline, 40000 points at a time
            for i in range(0, len(observed_times), 40000):
                print '..timeline chunk', i, min((i+40000), (len(observed_times)-1))
                timeline_chunk = observed_times[i:min((i+40000), (len(observed_times)-1))]

                obs_timeline_chunk = {}
                for t in timeline_chunk:
